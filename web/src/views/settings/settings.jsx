...
export default function Settings(props) {
  ...
  const [users, setUsers] = useState(reduxState.users)
  ...
  const save = () => {
    ...
    if (!users.some(user => user.login === input)) {
      gimletClient.saveUser(input)
        .then(saveUserResponse => {
          ...
          toast.update(progressToastId.current, {
            render: <Success header="User saved" />,
      ...
      toast.update(progressToastId.current, {
        render: <Error header="Error" message="User already exists" />,
        className: "bg-red-50 shadow-lg p-2",
        bodyClassName: "p-2",
        progressClassName: "!bg-red-200",
        autoClose: 5000
  ...
  const deleteUser = (login) => {
    ...
    gimletClient.deleteUser(login)
      .then(() => {
        toast.update(progressToastId.current, {
          render: <Success header="User deleted" />,
          className: "bg-green-50 shadow-lg p-2",
          bodyClassName: "p-2",
          autoClose: 3000,
        });
  ...
  const sortedUsers = sortAlphabetically(users);
