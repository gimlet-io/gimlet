...
func Test_write(t *testing.T) {
	...
	t.Run("Should write a file", func(t *testing.T) {
		...
		ioutil.WriteFile(fileToWrite.Name(), []byte("dummyContent"), os.FileMode(0644))
		args := strings.Split("gimlet gitops write", " ")
		args = append(args, "--gitops-repo-path", gitopsRepoPath)
		args = append(args, "--env", env)
		args = append(args, "--app", app)
		...
		lastCommit, err := repo.CommitObject(head.Hash())
		assert.NoError(t, err)
		expectedMessage := fmt.Sprintf("[Gimlet] %s/%s %s", env, app, "")
		assert.Equal(t, expectedMessage, lastCommit.Message)
	})
	...
	t.Run("Should write a folder", func(t *testing.T) {
		...
		ioutil.WriteFile(filepath.Join(dirToWrite, "dummy"), []byte("dummyContent"), os.FileMode(0644))
		args := strings.Split("gimlet gitops write", " ")
		args = append(args, "--gitops-repo-path", gitopsRepoPath)
		args = append(args, "--env", env)
		args = append(args, "--app", app)
	...
	t.Run("Should split a templated Helm manifest to multiple files", func(t *testing.T) {
		...
		ioutil.WriteFile(fileToWrite.Name(), []byte(manifestStr), os.FileMode(0644))
		args := strings.Split("gimlet gitops write", " ")
		args = append(args, "--gitops-repo-path", gitopsRepoPath)
		args = append(args, "--env", env)
		args = append(args, "--app", app)
		...
		for k, v := range scoreBoard {
			assert.True(t, v, "Couldn't find file ", k)
		}
