...
func (db *Store) UpdateEventStatus(id string, eventType string, status string, desc string, gitopsStatusString string, results string) error {
	stmt := sql.Stmt(db.driver, sql.UpdateEventStatus)
	_, err := db.Exec(stmt, eventType, status, desc, gitopsStatusString, results, id)
	return err
}
